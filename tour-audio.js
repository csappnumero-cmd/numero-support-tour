(function(){
  ...
  function showStartGate(){
    ...
    gate.style.cssText =
      ...
      'background:linear-gradient(135deg,#071118,#101b24);font-family:Arial,Helvetica,sans-serif;color:#fff;';

    var box = document.createElement('div');
    box.style.cssText =
      'width:min(520px,88vw);padding:34px 28px;border-radius:22px;text-align:center;' +
    ...
    brandWrap.style.cssText = 'display:flex;justify-content:center;align-items:center;margin-bottom:26px;';

    var brandImg = document.createElement('img');
    brandImg.src = detectLogoSrc();
    brandImg.alt = 'Numero eSIM';
    brandImg.style.cssText = 'max-width:min(280px,70vw);max-height:110px;width:auto;height:auto;display:block;object-fit:contain;filter:drop-shadow(0 8px 18px rgba(0,0,0,.25));';
    ...
    btn.style.cssText =
      'border:0;border-radius:14px;padding:15px 28px;background:#fff;color:#111;' +
      'font:900 15px Arial;letter-spacing:1.8px;cursor:pointer;';

    var hint = document.createElement('div');
    hint.textContent = 'Click once to start the presentation with audio';
    hint.style.cssText = 'font-size:12px;color:rgba(255,255,255,.58);margin-top:14px;';
    ...
    btn.addEventListener('click', function(){
      ...
      // Start the exact first MP3 that is already waiting.
      if (pendingFirstPlay) {
